...
const EMBED_ORIGIN = "https://tally.sarulean.com";
const TRASH_LIFETIME = 5 * 24 * 60 * 60 * 1000;
const REMOVED_SUPER_TYPES = new Set([
  "counters-grid",
  "top-bar-copy",
  "bottom-bar-copy",
]);
const encodeCounter = (counter) =>
...
const getGoals = (counter: AnyRecord): number[] => {
...
const countersEqual = (first, second) =>
...
function TallyApp() {
  ...
  const relativePath =
    basePath && location.pathname.startsWith(basePath)
      ...
      : location.pathname;
  const currentPath =
    `/${relativePath}`.replace(/\/{2,}/g, "/").replace(/\/$/, "") || "/";
  ...
  useEffect(() => {
    ...
    const isEmbedPage =
      location.pathname.replace(/\/$/, "").endsWith("/embed") ||
      query.has("embedData");
  ...
  if (currentPath === "/embed" || route.has("embedData")) {
    const embeddedCounter = decodeCounter(
      route.get("data") || route.get("embedData"),
    );
    return embeddedCounter ? (
      <EmbeddedCounter initial={embeddedCounter} params={route} />
    ) : (
...
const COUNTER_SUPER_PARTS: [string, string, boolean, boolean][] = [
...
function CounterSuperInspector({
  ...
  const [selected, setSelected] = useState(selectedFromStage || "title"),
    parts = value.parts || {},
      ...
      >
        ...
        <div className="sync-conflict-options">
          <button onClick={() => onChoose("device")}>
            ...
            <span>
              Upload these {deviceCount} counter{deviceCount === 1 ? "" : "s"}{" "}
              and replace the cloud copy.
            </span>
          </button>
      ...
      <div className="account-view">
        ...
        {!deleting ? (
          ...
          <div className="delete-account-panel">
            ...
            <p>
              This removes the account, cloud counters, and counters saved in
              this browser. Type <strong>DELETE</strong> to continue.
            </p>
            <input
              value={deleteText}
