...
export function validateBackup(raw: unknown, expectedScope?: BackupScope) {
  ...
  const preferenceKeys: string[] = value.scope === "all" ? ["density", "columns", "numberSize", "showBounds", "animations", "defaultColor", "trashEnabled", "syncTrash"] : [...PRESENTATION_KEYS];
  ...
  const preferenceValues: AnyRecord = { density: ["compact", "comfortable", "spacious"], columns: ["auto", "2", "3", "4"], numberSize: ["small", "standard", "large"], showBounds: "boolean", animations: "boolean", defaultColor: /^#[0-9a-f]{6}$/i, trashEnabled: "boolean", syncTrash: "boolean" };
